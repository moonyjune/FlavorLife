...
public class NetworkImageView extends ImageView {
				...
				new ImageListener() {
					...
					public void onResponse(final ImageContainer response,
						...
						if (response.getBitmap() != null) {
							setBitmapWithCustomeSize(response.getBitmap());
	...
	protected void setBitmapWithCustomeSize(Bitmap bitmap){
		setImageBitmap(bitmap);
		if (bitmap.getHeight() > 4000) {
			// layoutParams.height = 4000;
			// layoutParams.width = 4000 * getWidth()
			// / response.getBitmap().getHeight();
			bitmap=Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), 4000);
//			setImageBitmap(scaleDown(response.getBitmap(),response.getBitmap().getHeight(),true));
		}
